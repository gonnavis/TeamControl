...
vs.removeQueryStringByName=function(parameter) {
...
vs.randoms_in_arr = function(len, arr) {
  if (len > arr.length) {
    len = arr.length
  }
  let randoms = []
...
vs.sort_kv = function(array, key, is_reverse) {
  /*
    array = [
      {key:'abc'},
      {key:'def'},
  ...
  */
  let compare = function(a, b) {
    ...
    return (
      is_reverse ? (comparison * -1) : comparison
    );
  };
  array.sort(compare)
}
vs.delay = function(duration) {
  return new Promise(resolve => {
    setTimeout(() => resolve(), duration)
  })
}
vs.get_total_offset=function(dom){
  let total_offset={
    top:0,
    left:0,
  }
  recur(dom)
  function recur(node) {
    total_offset.top += node.offsetTop
    total_offset.left += node.offsetLeft
    if (node.offsetParent) {
      recur(node.offsetParent)
    }
  }
  return total_offset
}
